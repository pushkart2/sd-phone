...
The home-screen message preview, music strip, Your apps / All apps caption, and bottom Home / Apps / Messages navigation bar have been removed. Icons begin higher on the screen; Coast fits six rows (24 apps) on the first page. All 33 registry apps use a compact four-column grid with iPhone-style horizontal pages, not a vertically scrolling home grid. Swipe left/right on touch screens or drag with a mouse; pages snap into place. Click the page dots or focus the grid and use left/right arrow keys to navigate. Rows are packed to fit each preview without clipping or vertical scrolling, and returning from an app preserves the home page. Icons retain their original size and matching graphic marks. Removed apps are not included. These are preview tiles, not new production apps. The controls above the phones remain available for comparing example screens.
